...
class HardwareSecurityModuleGroup (models.Model, GenBase):
    pb_class = HardwareSecurityModuleGroupProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'hardwaresecuritymodulegroup-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        super(HardwareSecurityModuleGroup, self).save(*args, **kwargs)
    ...
    def delete(self):
        ...
        super(HardwareSecurityModuleGroup, self).delete()

    def serialize(self):
        return self.protobuf().SerializeToString()

    def _protobuf(self):
        pb = HardwareSecurityModuleGroupProto()
    ...
    tenant_ref = models.ForeignKey(Tenant, null=False, blank=False, related_name="hardwaresecuritymodulegroup_tenant_ref", on_delete=models.PROTECT)
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.tenant_uuid': ('belongs_to', u'Tenant', 'bb63261d0b1c466e'),
    }
    sensitive_fields = {
        'root.hsm.sluna.server.partition_passwd',
        'root.hsm.sluna.node_info.client_priv_key',
        'root.hsm.cloudhsm.crypto_user_password',
    }
    pb_check_fields = {
        'root.hsm.sluna.node_info.client_priv_key': 'f_immutable',
        'root.hsm.sluna.node_info.session_major_number': 'f_immutable',
        'root.hsm.sluna.ha_group_num': 'f_immutable',
        'root.hsm.sluna.node_info.chrystoki_conf': 'f_immutable',
        'root.hsm.sluna.node_info.client_cert': 'f_immutable',
        'root.hsm.cloudhsm.client_config': 'f_readonly',
        'root.hsm.cloudhsm.mgmt_config': 'f_readonly',
        'root.hsm.sluna.node_info.session_minor_number': 'f_immutable',
        'root.hsm.sluna.server_pem': 'f_immutable',
    }
