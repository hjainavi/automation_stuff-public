...
NAT_POLICY_ACTION_TYPE_DYNAMIC_IP_PORT = 1
NatPolicyActionTypeChoices = (
    (NAT_POLICY_ACTION_TYPE_DYNAMIC_IP_PORT, 'NAT_POLICY_ACTION_TYPE_DYNAMIC_IP_PORT'),
)
...
import time
from avi.protobuf.nat_policy_pb2 import NatPolicy as NatPolicyProto

from models_match import IpAddrGroup
from models_common import Tenant


class NatPolicy (models.Model, GenBase):
    pb_class = NatPolicyProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'natpolicy-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        super(NatPolicy, self).save(*args, **kwargs)
    ...
    tenant_ref = models.ForeignKey(Tenant, null=True, blank=True, related_name="natpolicy_tenant_ref", on_delete=models.PROTECT)
    group_refs = models.ManyToManyField(IpAddrGroup, related_name="natpolicy_ipaddrgroup", through="NatPolIpAddGro")
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.rules.match.destination_ip.group_uuids': ('refers_to', u'IpAddrGroup', 'e658f892357a4d05'),
        'root.tenant_uuid': ('belongs_to', u'Tenant', '7588c9aa64c44c06'),
        'root.rules.match.source_ip.group_uuids': ('refers_to', u'IpAddrGroup', 'e961ffd5644248c4'),
    }
    sensitive_fields = {
    }
    pb_check_fields = {
...
class NatPolIpAddGro (models.Model):
    natpolicy = models.ForeignKey(NatPolicy, null=True, blank=True, on_delete=models.PROTECT) 
