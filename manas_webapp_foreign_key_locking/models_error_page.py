...
ErrorPageFormatChoices = (
    (ERROR_PAGE_FORMAT_HTML, 'ERROR_PAGE_FORMAT_HTML'),
    (ERROR_PAGE_FORMAT_JSON, 'ERROR_PAGE_FORMAT_JSON'),
)
...
import time
from avi.protobuf.error_page_pb2 import ErrorPageBody as ErrorPageBodyProto



class ErrorPageBody (models.Model, GenBase):
    pb_class = ErrorPageBodyProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'errorpagebody-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        super(ErrorPageBody, self).save(*args, **kwargs)
    ...
    def delete(self):
        ...
        super(ErrorPageBody, self).delete()

    def serialize(self):
        return self.protobuf().SerializeToString()

    def _protobuf(self):
        pb = ErrorPageBodyProto()
    ...
    tenant_ref = models.ForeignKey(Tenant, null=True, blank=True, related_name="errorpagebody_tenant_ref", on_delete=models.PROTECT)
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.tenant_uuid': ('belongs_to', u'Tenant', '22765c5764474d44'),
    }
    sensitive_fields = {
    }
    pb_check_fields = {
...
import time
from avi.protobuf.error_page_pb2 import ErrorPageProfile as ErrorPageProfileProto



class ErrorPageProfile (models.Model, GenBase):
    pb_class = ErrorPageProfileProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'errorpageprofile-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        if (hasattr(self, '_last_modified') and
            ...
            if hasattr(self, 'json_data'):
                self.json_data['_last_modified'] = self._last_modified
        super(ErrorPageProfile, self).save(*args, **kwargs)
    ...
    def delete(self):
        delete_model_object(self)
        super(ErrorPageProfile, self).delete()

    def serialize(self):
        return self.protobuf().SerializeToString()

    def _protobuf(self):
        pb = ErrorPageProfileProto()
        if hasattr(self, 'json_data') and self.json_data:
            updated_json = copy.deepcopy(self.json_data)
            transform_json_refs_to_uuids(updated_json, refs=self.references)
            json2pb(pb, updated_json, replace=True)
    ...
    json_data = AviJsonField()
    name = models.TextField(null=False, blank=False)
    tenant_ref = models.ForeignKey(Tenant, null=True, blank=True, related_name="errorpageprofile_tenant_ref", on_delete=models.PROTECT)
    error_page_body_ref_61973ac37f3649fc_ref = models.ForeignKey(ErrorPageBody, null=True, blank=True, related_name="errorpage_error_page_body_ref_61973ac37f3649fc", on_delete=models.PROTECT)
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.error_pages.error_page_body_uuid': ('refers_to', u'ErrorPageBody', '61973ac37f3649fc'),
        'root.tenant_uuid': ('belongs_to', u'Tenant', '7893c84ccae341ab'),
    }
    sensitive_fields = {
    }
    pb_check_fields = {
