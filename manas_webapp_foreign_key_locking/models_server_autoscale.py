...
import time
from avi.protobuf.server_autoscale_pb2 import AutoScaleLaunchConfig as AutoScaleLaunchConfigProto

from models_common import Tenant


class AutoScaleLaunchConfig (models.Model, GenBase):
    pb_class = AutoScaleLaunchConfigProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'autoscalelaunchconfig-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        super(AutoScaleLaunchConfig, self).save(*args, **kwargs)
    ...
    def delete(self):
        ...
        super(AutoScaleLaunchConfig, self).delete()

    def serialize(self):
        return self.protobuf().SerializeToString()

    def _protobuf(self):
        pb = AutoScaleLaunchConfigProto()
    ...
    tenant_ref = models.ForeignKey(Tenant, null=False, blank=False, related_name="autoscalelaunchconfig_tenant_ref", on_delete=models.PROTECT)
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.tenant_uuid': ('belongs_to', u'Tenant', 'a3765e07726e4b62'),
    }
    sensitive_fields = {
    }
    pb_check_fields = {
...
import time
from avi.protobuf.server_autoscale_pb2 import ServerAutoScalePolicy as ServerAutoScalePolicyProto

from models_common import Tenant


class ServerAutoScalePolicy (models.Model, GenBase):
    pb_class = ServerAutoScalePolicyProto
    json = True
    random_uuid = True

    @staticmethod
    def gen_uuid(id=1):
        return 'serverautoscalepolicy-%s' % uuid.uuid4()
    ...
    def save(self, update_time=False, *args, **kwargs):
        ...
        if (hasattr(self, '_last_modified') and
            ...
            if hasattr(self, 'json_data'):
                self.json_data['_last_modified'] = self._last_modified
        super(ServerAutoScalePolicy, self).save(*args, **kwargs)
    ...
    def delete(self):
        delete_model_object(self)
        super(ServerAutoScalePolicy, self).delete()

    def serialize(self):
        return self.protobuf().SerializeToString()

    def _protobuf(self):
        pb = ServerAutoScalePolicyProto()
        if hasattr(self, 'json_data') and self.json_data:
            updated_json = copy.deepcopy(self.json_data)
            transform_json_refs_to_uuids(updated_json, refs=self.references)
            json2pb(pb, updated_json, replace=True)
    ...
    json_data = AviJsonField()
    name = models.TextField(null=False, blank=False)
    tenant_ref = models.ForeignKey(Tenant, null=False, blank=False, related_name="serverautoscalepolicy_tenant_ref", on_delete=models.PROTECT)
    _last_modified = models.CharField(max_length=16, null=True, blank=True)

    references = {
        'root.scalein_alertconfig_uuids': ('hyperlink_to', u'AlertConfig', 'f4186229a62941ca'),
        'root.tenant_uuid': ('belongs_to', u'Tenant', '38c1af60dc5f4fd4'),
        'root.scaleout_alertconfig_uuids': ('hyperlink_to', u'AlertConfig', 'b80a9285f52f47b9'),
    }
    sensitive_fields = {
    }
    pb_check_fields = {
