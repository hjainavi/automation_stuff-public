...
def connect(vcenter_ip,user,pwd,exit_on_error=True):
    try:
        si= SmartConnectNoSSL(host=vcenter_ip, user="root", pwd="vmware")#, sslContext=s)
...
if len(sys.argv)==1 or (len(sys.argv)==2 and sys.argv[1]=='with_host_datastore'):
    ...
    vmware_host = "10.140.0.10"
   
    si = connect(vmware_host, user="root", pwd="vmware")#, sslContext=s)
    ...
    for host in cluster_obj.host:
        ...
        for datastore in host.datastore:
            ...
            if (free_memory_in_host_in_MB > (ova_memory_spec_in_MB + 1024)) and (free_disk_space_in_datastore_in_MB > (ova_disk_spec_in_MB + ova_memory_spec_in_MB/2.0 + 1024)):
                host_datastore_compatible.append([host.name,str(free_memory_in_host_in_GB)+'GB',datastore.name,str(free_disk_space_in_datastore_in_GB)+'GB','COMPATIBLE'])
            else:
...
def check_if_ip_is_free(si,datacenter_obj,ip):
    ...
    if vm:
        print "Ip = %s is already being used by %s"%(ip,vm.__repr__())
        return False
    else:
        return True
...
def generate_controller_from_ova():
    vm_type = "controller"
    vcenter_ip = raw_input("Vcenter IP ? [Default: 10.140.0.10] :") or '10.140.0.10'
    si = connect(vcenter_ip, user="root", pwd="vmware")#, sslContext=s)
    datacenter = raw_input("Datacenter Name ? [Default: Bangalore] :") or 'Bangalore'
    datacenter_obj = get_datacenter_obj(si,datacenter)
    cluster_name = raw_input("Cluster ? [Default: Netgear] :") or 'Netgear'
    ...
    while True:
        ...
        if mgmt_ip:
            if check_if_ip_is_free(si,datacenter_obj,mgmt_ip):
                break
    mask = raw_input("Network Mask ? [Default: 20] :") or '20'
    gw_ip = raw_input("Gateway IP ? [Default: 10.140.16.1] :") or '10.140.16.1'
    while True:
